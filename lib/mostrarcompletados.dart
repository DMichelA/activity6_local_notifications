...
class mostrarCompletados extends StatefulWidget {
  @override
  _mostrarCompletados createState() => new _mostrarCompletados();
}

class _mostrarCompletados extends State<mostrarCompletados> {
  //VARIABLES REFERENTES AL MANEJO DE LA BD

  Future<List<completados>> recordatorios;
  List matricula;
  int contador;
  ...
  void refreshList() async {
    setState(() {
      recordatorios = dbHelper.getRecordatorios4();
    });
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
      ...
      appBar: new AppBar(
        ...
        centerTitle: true,
        title: Text("Recordatorios Completados",
            style: TextStyle(
                color: Colors.white,
                fontWeight: FontWeight.bold,
                fontSize: 20)),
...
//                                        builder: (context) => homePage3()));
                            ...
                            Row(
                              children: <Widget>[
                                ...
                                FlatButton.icon(
                                  onPressed: () {
                                    dbHelper.delete4(snapshot.data[index].id);
                                    _showSnackbar(context,
                                        "Los recordatorios completados se eliminaron correctamente");
                                    refreshList();
                                  },
                                  color: Colors.red,
