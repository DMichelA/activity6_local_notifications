...
class actualizacionProgramado extends StatefulWidget {
  int id;
  actualizacionProgramado(this.id);
  @override
  _actualizacionProgramado createState() => new _actualizacionProgramado(this.id);
}

class _actualizacionProgramado extends State<actualizacionProgramado> {
  //VARIABLES REFERENTES AL MANEJO DE LA BD
  int id;
  _actualizacionProgramado(this.id);
  ...
  void enviaraclase() async {
    listarec = await dbHelper.decodificar3();
    ...
    Navigator.push(
        ...
        MaterialPageRoute(
            builder: (context) =>
                mostrarNotificacion(idn, nombren, fechan, horan)));
                      ...
                      onPressed: () {
                        ...
                        DatePicker.showDateTimePicker(context,
                          ...
                          print(hora);
                            ...
                            currentTime: DateTime.utc(2020, 07, 01, 00, 00, 00),
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
      ...
      appBar: new AppBar(
        ...
        centerTitle: true,
        title: Text("Actializar Recordatorio",
            style: TextStyle(
                color: Colors.white,
                fontWeight: FontWeight.bold,
                fontSize: 20)),
  ...
  void insert() {
    ...
    if (isUpdating == false) {
      ...
      if (formKey.currentState.validate()) {
        ...
        Recordatorio rec =
            Recordatorio(id, nombre.toUpperCase(), fecha, hora, descripcion);
        if (nombre == null ||
            nombre == "" ||
            fecha == "" ||
            fecha == null ||
...
class mostrarNotificacion extends StatefulWidget {
  mostrarNotificacion(this.id, this.nombre, this.fecha, this.hora);
  ...
  @override
  _mostrarNotificacionState createState() =>
      _mostrarNotificacionState(this.id, this.nombre, this.fecha, this.hora);
}
...
//    var reprogram = now.add(Duration(hours: 00, minutes: 00, seconds: 10));
    var time = DateTime.parse(fecha + "T" + hora + ".000");
    print("la notif con id:" + id.toString() + "es" + time.toString());
    //id aleatorio tiene que ser pero por ahora estatico

    await flutterLocalNotificationsPlugin.schedule(
        id, nombre, fecha, time, platformChannelSpecifics,
        payload: nombre);

    ///ver la hora programada
    Fluttertoast.showToast(
        msg: 'Scheduled at time $time',
...
//        context, new MaterialPageRoute(builder: (context) => new SecondPage(payload: payload )));
  ...
  Widget build(BuildContext context) {
    // TODO: implement build
    return mostrarTarjetaProgramado();
  }
